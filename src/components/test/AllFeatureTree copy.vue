<template>
  <div>
    <button @click="displayEpicPageOne">Toggle Component One</button>
  <v-treeview dense activatable color="green" :items="items" :item-key =name>
    <template v-slot:default="{ item }">
      <v-treeview-node :item="item">
        <template  v-if="item.name === 'Epic 1'">
          <button v-on:click.native="displayEpicPageOne">{{ item.name }}</button>
        </template>
        <template v-else>
          {{ item.name }}
        </template>
