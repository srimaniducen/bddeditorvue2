<template class="main1">
    <v-card class="card">
      <v-container><v-col cols="14" md="10"> <label class="text">Story title</label>
          <v-text-field outlined dense maxlength="12" tabindex="4">
          </v-text-field>
        </v-col>
          ...
          <b-form-tags v-model="value" style="display:contents;">
            <template v-slot="{ tags, inputAttrs, inputHandlers, tagVariant, removeTag }">
              ...
              <div class="d-inline-block tasks-area "
                ...
                <b-form-tag style="color:black;border:1px solid;margin:10px" v-for="tag in tags" @remove="removeTag(tag)" :key="tag" :title="tag"
  ...
  export default {
    data: () => ({
      ...
      items: [
        { text: "To Do", icon: "fas fa-circle" },
        { text: "Process", icon: "fas fa-trash" },
        { text: "Completed", icon: "fas fa-edit" }
      ],
      rows: []
    }),
    methods: {
